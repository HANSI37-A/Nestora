    ...
    <div className="min-h-screen bg-[#F9F7F2] text-[#1A1A1A] font-sans antialiased select-none px-6 py-16 sm:px-12 lg:px-24">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
        {showrooms && showrooms.length > 0 ? (
          showrooms.map((room) => (
            <div key={room._id} className="group flex flex-col space-y-4 bg-white/40 p-4 rounded-xl border border-transparent hover:border-[#1A1A1A]/5 hover:bg-white transition-all duration-500">
              {/* Media Display Component Box */}
              <div className="w-full aspect-[3/2] overflow-hidden rounded-lg bg-gray-100 relative">
                <img
                  src={room.image}
                  alt={room.name}
                  ...
                  onError={(e) => { e.target.src = "https://placehold.co/600x400?text=Asset+Render+Missing"; }}
                />
              ...
              <div className="flex flex-col flex-1 pt-2">
                ...
                <p className="text-xs text-[#A8A29E] leading-relaxed font-light text-justify line-clamp-3 group-hover:text-[#1A1A1A] transition-colors duration-500">
                  {room.description}
                </p>
              </div>
            </div>
